...
#    video_data['likes'] = int(re.sub('[^0-9]', '',soup.select('.likes-count')[0].get_text().split()[0]))
#    video_data['dislikes'] = int(re.sub('[^0-9]', '',soup.select('.dislikes-count')[0].get_text().split()[0]))
    return video_data
...
#    for video_page_url in video_page_urls:
    print get_video_data(video_page_urls[0])
